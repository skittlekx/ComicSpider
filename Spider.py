...
class MHDSpider_Phone(Base):
    ...
    def comics_parse(self,url,r_step):
        try:
            ...
            # soup = BeautifulSoup(content, 'html5lib')
            # page_selects = soup.find('select',id='page_select')
            pageinfo = driver.find_element_by_class_name('BarTit')

            text = pageinfo.text
            text = text[text.find('/') + 1:text.find(')')]
            #select = Select(driver.find_element_by_id('page_select'))
            count = int(text)#len(select.options)
            ...
            while(1):
                ...
                # img_html = soup.find('div',id='images')
                # img_tag = img_html.find('img')
                img_info = driver.find_element_by_id('chapter-view').find_element_by_tag_name('mip-img')
            
                #img_url = img_tag['src']
                img_url = img_info.get_attribute('src')
                #img_page = img_info.get_attribute('data-index')
                img_page_n = img_page_n + 1
                img_page = str(img_page_n)
                img_page = img_page.zfill(3)
                if(img_page_n not in step):
                    actions = driver.find_element_by_class_name('control_bottom').find_element_by_id('action').find_elements_by_tag_name('li')
                    for action in actions:
                        if(action.text == '下一页'):
                            nexturl = action.find_element_by_tag_name('mip-link').get_attribute('href')
                            driver.get(nexturl)
                            break
                    continue
                self.save_image(img_page,img_url)
